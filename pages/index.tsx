...
function gerarRelatorio(preData: IPreData) {
  ...
  preData.sitePermissions.forEach(item => {
    ...
    consoleGroup('Permissions', Permissions);
...
function reportPermissions(perm: ISiteDefinition, preData: IPreData) {
  ...
  const groupReports = groups.map(g => {
    ...
    const permOnSiteNames = permOnSite ? permOnSite.RoleDefinitionBindings.map(r => r.Name).sort() : null
    const permOnSiteIds = permOnSite ? permOnSite.RoleDefinitionBindings.map(r => r.Id).sort() : null
